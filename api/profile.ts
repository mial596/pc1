...
async function handleGet(req: VercelRequest, res: VercelResponse, db: Db, userId: string, decodedToken: DecodedToken) {
    ...
    if (!userProfile) {
        ...
        const repairedData = {
            ...
            catPass: { ...initialData.catPass, ...(existingData.catPass || {}) },
            dailyMissions: { ...initialData.dailyMissions, ...(existingData.dailyMissions || {}) },
        };
        ...
        if (!userProfile.username && userProfile.email) {
            ...
            await users.updateOne({ _id: userId as any }, { $set: { username: userProfile.username } });
        }
...
async function handlePut(req: VercelRequest, res: VercelResponse, users: any, userId: string) {
    ...
    const existingUser = await users.findOne({ username: username, _id: { $ne: userId as any } });
    ...
    await db.collection('trades').updateMany({ fromUserId: userId }, { $set: { fromUsername: username } });
    await db.collection('trades').updateMany({ toUserId: userId }, { $set: { toUsername: username } });
...
async function updatePublicPhrases(db: Db, userId: string, newPhrases: Phrase[]) {
    ...
    for (const phrase of publicPhraseIntents) {
        ...
        const userProfileForPhrase = await resolveProfilePictureUrl(db, user);
        ...
        await publicPhrasesCollection.updateOne(
            ...
            { $set: { 
                ...
                profilePictureUrl: userProfileForPhrase,
            }, $setOnInsert: { likes: [], comments: [], commentCount: 0 } },
            { upsert: true }
        );
    }
}
