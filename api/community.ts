    ...
    if (req.method === 'POST') {
        try {
            ...
            if (action === 'report') {
                ...
                const author = await users.findOne({ _id: contentAuthorId as any });
...
async function getPublicFeed(res: VercelResponse, db: any, currentUserId: string) {
    ...
    const userMap = new Map(usersWithPics.map((u: any) => [u._id.toString(), { pic: u.profilePictureUrl, role: u.role }]));
    ...
    const response = feedPhrases.map((p: any) => ({
        ...
        role: userMap.get(p.userId.toString())?.role || 'user',
        userId: p.userId,
        profilePictureUrl: userMap.get(p.userId.toString())?.pic,
        comments: p.comments || [],
        commentCount: p.comments?.length || 0,
    }));
    return res.status(200).json(response);
