...
async function handleGet(res: VercelResponse, db: Db, userId: string) {
    ...
    const friendsCursor = allFriendIds.length > 0
      ? await users.find({ _id: { $in: allFriendIds as any } }).project({ username: 1, isVerified: 1, role: 1, data: { profilePictureId: 1 } }).toArray()
      : [];
    ...
    const requestsCursor = requestIds.length > 0
      ? await users.find({ _id: { $in: requestIds as any } }).project({ username: 1, data: { profilePictureId: 1 } }).toArray()
      : [];
...
async function handlePut(req: VercelRequest, res: VercelResponse, db: Db, userId: string) {
    ...
    const users = db.collection('users');
    await users.updateOne({ _id: userId as any }, { $pull: { "data.friendRequestsReceived": targetUserId } as any);
    await users.updateOne({ _id: targetUserId as any }, { $pull: { "data.friendRequestsSent": userId } as any);
