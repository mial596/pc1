...
async function handler(req: VercelRequest, res: VercelResponse) {
    try {
        ...
        switch (req.method) {
            ...
            case 'POST':
                return await handlePost(req, res, db, userId); // Add friend or Like Phrase
            case 'PUT':
...
async function handlePost(req: VercelRequest, res: VercelResponse, db: any, userId: string) {
    const { targetUserId, action, publicPhraseId } = req.body;
    const users = db.collection('users');
    ...
    await users.updateOne({ _id: userId }, { $addToSet: { friendRequestsSent: targetUserId } });
    await users.updateOne({ _id: targetUserId }, { $addToSet: { friendRequestsReceived: userId } });
...
async function handlePut(req: VercelRequest, res: VercelResponse, users: any, userId: string) {
    ...
    await users.updateOne({ _id: userId }, { $pull: { friendRequestsReceived: targetUserId } });
    await users.updateOne({ _id: targetUserId }, { $pull: { friendRequestsSent: userId } });
    
    if (action === 'accept') {
        await users.updateOne({ _id: userId }, { $addToSet: { friends: targetUserId } });
        await users.updateOne({ _id: targetUserId }, { $addToSet: { friends: userId } });
    }
...
async function handleDelete(req: VercelRequest, res: VercelResponse, users: any, userId: string) {
    ...
    await users.updateOne({ _id: userId }, { $pull: { friends: targetUserId } });
    await users.updateOne({ _id: targetUserId }, { $pull: { friends: userId } });
