...
async function handleGet(req: VercelRequest, res: VercelResponse, db: Db) {
    ...
    if (resource === 'settings') {
        let settings = await db.collection<any>('game_settings').findOne({ _id: 'main' });
        if (!settings) {
            settings = { _id: 'main', rarityValues: { common: 10, rare: 50, epic: 200 } };
            await db.collection('game_settings').insertOne(settings as any);
        }
...
async function handlePost(req: VercelRequest, res: VercelResponse, db: any) {
    ...
    if (action === 'censorPhrase') {
        ...
        if (phraseToDelete) {
            await db.collection('public_phrases').deleteOne({ _id: new ObjectId(publicPhraseId) as any });
            await db.collection('users').updateOne(
                { _id: phraseToDelete.userId, 'data.phrases.id': phraseToDelete.phraseId } as any,
                { $set: { 'data.phrases.$.visibility': 'private' } }
            );
        }
        return res.status(200).json({ success: true });
    }
    ...
    if (action === 'saveSettings') {
        ...
        await db.collection('game_settings').updateOne({ _id: 'main' as any }, { $set: settings }, { upsert: true });
        return res.status(200).json({ success: true });
    }
