...
interface PublicFeedProps {
    ...
    onReport: (data: {type: 'phrase', contentId: string}) => void;
    onOpenComments: (phrase: PublicProfilePhrase) => void;
}

const PublicFeed: React.FC<PublicFeedProps> = ({ currentUserId, onProfileClick, onReport, onOpenComments }) => {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <h1 className="text-3xl font-black text-ink text-center mb-6 font-cartoon">Community Feed</h1>
            {feed.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {feed.map((phrase) => (
                        <div key={phrase.publicPhraseId} className="card-themed p-3 flex flex-col">
                            <header className="flex items-center justify-between gap-3 mb-2">
                                <button onClick={() => onProfileClick(phrase.username!)} className="flex items-center gap-2 overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-1">
                                       <span className="font-bold text-base text-ink hover:underline truncate">@{phrase.username}</span>
                                       <UserBadges role={phrase.role!} isVerified={!!phrase.isUserVerified} />
                                    </div>
                                </button>
                                {phrase.userId !== currentUserId &&
                                    <button onClick={() => onReport({ type: 'phrase', contentId: phrase.publicPhraseId })} className="font-bold text-xs text-ink/40 hover:text-accent flex-shrink-0">
                                        ...
                                    </button>
                                }
                            </header>
                            ...
                            <div className="flex items-center justify-between gap-2 mt-1">
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className={`btn-themed !p-2 bg-surface hover:bg-rose-900/50 active:bg-rose-800/50 text-2xl ${phrase.isLikedByMe ? 'text-rose-500' : 'text-ink/50'}`}
                                ...
                                <button onClick={() => onOpenComments(phrase)} className="btn-themed !py-1 !px-3 bg-surface-darker">
                                    💬 {phrase.commentCount} Comentarios
                                </button>
                            </div>
                        </div>
                    ))}
