...
const PublicFeed: React.FC<PublicFeedProps> = ({ currentUserId, onProfileClick }) => {
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-12">
            <h1 className="text-3xl font-black text-liver text-center">Community Feed</h1>
                    ...
                    <div key={phrase.publicPhraseId} className={rotationClass}>
                        <div className="card-cartoon has-tape p-4">
                            <header className="flex items-center gap-3 mb-3">
                                <button onClick={() => onProfileClick(phrase.username!)} className="font-bold text-lg text-liver hover:underline">
                                    @{phrase.username}
                                </button>
                                {phrase.isUserVerified && <VerifiedIcon className="w-5 h-5 text-blue-500" title="Verified User" />}
                            </header>
                            <div className="aspect-square bg-slate-200 rounded-lg overflow-hidden border-2 border-liver/20 mb-3">
                            ...
                            <div className="flex items-center gap-3 mt-2">
                                 ...
                                 <span className="font-black text-lg text-liver/90">{phrase.likeCount}</span>
                            </div>
                        </div>
                    </div>
                );
