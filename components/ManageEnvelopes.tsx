...
const initialFormState: Omit<Envelope, 'isFeatured'> & {isFeatured: boolean} = {
    id: '',
    name: '',
    baseCost: 100,
...
const ManageEnvelopes: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const token = await getAccessTokenSilently();
            await apiService.adminAddEnvelope(token, formState);
            setFormState(initialFormState);
            await fetchData(); // Refresh
        } catch (err) {
            setError('Failed to add new envelope. ID might already exist.');
        } finally {
    ...
    return (
        <div>
            <div className="card-themed p-4 mb-6">
                <h3 className="text-xl font-bold mb-4">Add New Envelope</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div><label className="font-bold text-sm">ID</label><input type="text" name="id" value={formState.id} onChange={handleInputChange} className="input-themed mt-1" /></div>
                    ...
                    <div className="flex items-center gap-4">
                        <label className="flex items-center gap-2 font-bold cursor-pointer"><input type="checkbox" name="isFeatured" checked={formState.isFeatured} onChange={handleInputChange} className="w-5 h-5 rounded text-primary focus:ring-primary"/> Is Featured?</label>
                    </div>

                    <button type="submit" disabled={isSubmitting} className="btn-themed btn-themed-primary w-full flex items-center justify-center gap-2">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {envelopes.map(env => (
                    <div key={env.id} className="card-themed p-4">
                        ...
                        <p className="text-xs mt-1 font-bold">Themes: {env.catThemePool.join(', ') || 'All'}</p>
                    </div>
                ))}
            </div>
        </div>
