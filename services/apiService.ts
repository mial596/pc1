...
// --- Shop ---
export const getShopData = (): Promise<{ envelopes: Envelope[], upgrades: GameUpgrade[], featured: ShopFeaturedItem[] }> => {
...
export const purchaseFeaturedCat = (token: string, catId: number): Promise<{ updatedProfile: UserProfile }> => {
    return apiRequest('/api/shop', 'POST', token, { action: 'purchaseFeaturedCat', catId });
};

export const purchasePremiumPass = (token: string): Promise<{ updatedProfile: UserProfile }> => {
    return apiRequest('/api/shop', 'POST', token, { action: 'purchasePremiumPass' });
};


// --- Codes ---
export const redeemCoinCode = (token: string, code: string): Promise<{ success: boolean; coinsAdded: number }> => {
    return apiRequest('/api/codes', 'POST', token, { code });
};
...
export const getMissions = (token: string): Promise<DailyMission[]> => {
    return apiRequest('/api/game?resource=missions', 'GET', token);
};

export const claimMissionReward = (token: string, missionId: string): Promise<{ updatedProfile: UserProfile }> => {
    return apiRequest('/api/game', 'POST', token, { action: 'claimMission', missionId });
};

export const completeRandomMission = (token: string): Promise<{ updatedProfile: UserProfile }> => {
    return apiRequest('/api/game', 'POST', token, { action: 'completeRandomMission' });
};

export const claimMission = (token: string, missionType: 'like_phrase' | 'open_envelope', amount: number): Promise<{success: boolean}> => {
     return apiRequest('/api/game', 'POST', token, { action: 'progressMission', missionType, amount });
}

export const getCatPassRewards = (token: string): Promise<PassReward[]> => {
    return apiRequest('/api/game?resource=catpass_rewards', 'GET', token);
};

export const claimCatPassReward = (token: string, level: number, type: 'free' | 'premium'): Promise<{ updatedProfile: UserProfile }> => {
    return apiRequest('/api/game', 'POST', token, { action: 'claimPassReward', level, type });
};
